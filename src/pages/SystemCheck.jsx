...
const SystemCheck = () => {
  ...
  const { userId, testId } = useSelector((state) => state.test);
  const navigate = useNavigate();

  const handleSystemCheckSubmit = async () => {
    if (!testId || !userId) {
      toast.error("Invalid test session. Please restart.");
      return;
    }
    ...
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/tests/system-check`,
        {
          test_id: testId,
          camera: cameraOn,
          microphone: micOn,
          speaker: speakerStatus.includes("Playing"),
          network_speed: "fast", // Optional: implement a network speed check if needed
        },
        {
          headers: {
      ...
      toast.success("System check passed. Redirecting to exam...");
      setTimeout(() => navigate("/image-verification"), 1500);
    } catch (err) {
      console.error("System check error:", err);
      const message =
        err?.response?.data?.message || "System check failed. Try again.";
      toast.error(message);
    }
  };
      ...
      <div className="flex-grow flex items-center justify-center p-4 mt-16 md:p-8">
        <div className="w-full bg-white rounded-2xl shadow-2xl p-6 md:p-10 flex flex-col gap-8">
          ...
          <p className="text-center text-gray-600 text-sm md:text-base">
            Before you start your exam, please make sure your camera,
            microphone, and speakers are working correctly.
          </p>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            {/* CAMERA */}
            <div
              className="flex-1 flex flex-col items-center text-center lg:text-left lg:items-start bg-indigo-50 rounded-xl p-6 
                  lg:h-[500px]">
              ...
              <p className="text-gray-700 text-sm mb-4">
                Click the button to enable your camera. Make sure your image
                appears below.
              </p>
              <button
              ...
              <div
                className="mt-4 rounded-lg overflow-hidden border shadow-inner bg-black 
            ...
            {/* SPEAKER */}
            <div
              className="flex-1 flex flex-col items-center text-center lg:text-left lg:items-start bg-yellow-50 rounded-xl p-6 
                  lg:h-[500px]">
            ...
            {/* MICROPHONE */}
            <div
              className="flex-1 flex flex-col items-center text-center lg:text-left lg:items-start bg-green-50 rounded-xl p-6 
                  lg:h-[500px]">
              ...
              <p className="text-gray-700 text-sm mb-4">
                Click start to enable your microphone. Speak into your mic to
                hear yourself live.
              </p>
              <button
          ...
          <div className="flex flex-col md:flex-row justify-center md:justify-end gap-4 mt-6">
            {/* <Link
              to="/instructions"
              className="bg-gray-600 cursor-pointer hover:bg-gray-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-300">
              Back
            </Link> */}
            
            <button
              onClick={handleSystemCheckSubmit}
              className="bg-indigo-600 cursor-pointer hover:bg-indigo-700 text-white font-semibold py-3 px-6 rounded-lg transition-transform transform hover:scale-[1.02] duration-300 shadow-md text-center">
              Proceed to Exam
            </button>
          </div>
        </div>
