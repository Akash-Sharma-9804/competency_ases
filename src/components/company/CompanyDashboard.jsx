...
import { Home, FilePlus, BarChart3, Users, User, HelpCircle, LogOut, Menu, X } from "lucide-react";
 ...
 // Initialize activeTab from localStorage or default to "dashboard"
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "results": return <CompanyResults />;
      case "candidates": return <Candidates />;
      case "profile": return <CompanyProfile />;
      case "help": return <CompanyHelp />;
      default: return null;
    }
  };
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      >
        <div className="p-6 border-b flex justify-between items-center">
          <div>
            <h1 className="text-xl font-bold text-indigo-600">Company Panel</h1>
          ...
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full cursor-pointer flex items-center space-x-2 px-4 py-2 rounded-lg text-left transition ${
                activeTab === item.id ? "bg-indigo-600 text-white" : "text-gray-700 hover:bg-indigo-50"
              }`}
            >
              {item.icon}
