...
const predefinedTimezones = [
  ...
  { value: 'Asia/Riyadh', label: 'Saudi Arabia (AST)' },  // ✅ matches Intl
 ...
 const [timezone, setTimezone] = useState(() => {
  ...
  const exists = predefinedTimezones.some(tz => tz.value === systemTZ);
  return exists ? systemTZ : 'Asia/Kolkata'; // fallback to India if not found
});

// ✅ Always put the user’s timezone first
const timezones = (() => {
  const systemTimeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
  const existing = predefinedTimezones.find(tz => tz.value === systemTimeZone);
  if (existing) {
    return [
      { ...existing, label: `${existing.label} (Your Timezone)` },
      ...predefinedTimezones.filter(tz => tz.value !== systemTimeZone),
    ];
  }
  return [
    { value: systemTimeZone, label: `${systemTimeZone} (Your Timezone)` },
    ...predefinedTimezones,
  ];
})();
...
useEffect(() => {
  const systemTZ = Intl.DateTimeFormat().resolvedOptions().timeZone;
  const exists = timezones.some(tz => tz.value === systemTZ);
  if (exists) {
    setTimezone(systemTZ);
  } else {
    setTimezone('Asia/Kolkata'); // fallback if system tz not in list
  }
}, [timezones]);
  ...
  const handleUpdate = async () => {
    try {
      await testAPI.updateTest(editForm.id, {
        ...editForm,
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-600 mt-1">
                    {filteredQuestions.length} of {viewQuestions.length}{" "}
                    questions
                  </p>
                </div>
              </div>
              ...
              {loadingTests ? (
                ...
                tests.map((t) => (
                  <tr
                    ...
                    {/* Status */}
                    <td className="py-4 px-6 font-poppins capitalize">
                      {t.status}
                    </td>
                  ...
                  <select
                    ...
                    {timezones.map((tz) => (
                      <option key={tz.value} value={tz.value}>
                        {tz.label} ({getCurrentOffset(tz.value)}) –{" "}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-green-100 text-sm">
                        Select candidates to assign this test
                      </p>
                    </div>
                  </div>
                  <button
                    ...
                    <div className="text-center py-12">
                      {searchTerm ? (
                        ...
                        <>
                          ...
                          <p className="text-gray-400 text-sm mt-1">
                            Add candidates to your company first
                          </p>
                        </>
                      ) : null}
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          disabled={filteredCandidates.every((c) =>
                            selectedCandidateIds.includes(c.user_id)
                          )}
                          ...
                          Select All{" "}
                          {searchTerm && `(${filteredCandidates.length})`}
                        </button>
                        <button
                          onClick={() => {
                            filteredCandidates.forEach((c) => {
